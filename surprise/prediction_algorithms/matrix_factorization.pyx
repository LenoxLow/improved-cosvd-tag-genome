...
class co_SVD_v6(AlgoBase):
    ...
    def sgd(self, trainset):
        ...
        #final_raw  = ratings[ratings.ref_key.isin(data_ref)]
        #extra_data = ratings[(~ratings.ref_key.isin(data_ref)) & (ratings.rating == 0) & ratings.userId.isin(raw_user) & ratings.movieId.isin(raw_item)]
        #raw_data = pd.concat([final_raw, extra_data], ignore_index=True)
        #raw_data = ratings[ratings.ref_key.isin(data_ref) | (~np.isnan(ratings.tid))]
        ...
        #tag_data = raw_data[raw_data.rating == 0].reset_index()
        #raw_data = raw_data[raw_data.rating != 0].reset_index()
